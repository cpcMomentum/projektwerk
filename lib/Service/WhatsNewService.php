...
/**
 * „Was ist neu?"-Fenster (Baustein 0b, Rollout nach RechnungsWerk #308).
 *
 ...
 *
 * **App-spezifisch fuer ProjektWerk (Konzept Abschnitt 3): Gast- und
 * Kundennutzer sehen das Fenster nie.** Kunden unserer Kunden bekommen keine
 * Produktmeldungen. Anders als der Pilot RechnungsWerk hat ProjektWerk diesen
 * Fall — deshalb steht der Gast-Riegel {@see AccountType} hier vor jeder
 * Ausgabe. Der Rest der Klasse bleibt die 1:1-Vorlage der Flotte.
 */
class WhatsNewService {
	 ...
	 * gesetzt von {@see \OCA\Projektwerk\Repair\WhatsNewBootstrap}.
	...
	public function getPending(string $userId): array {
		...
		// Gast-/Kundennutzer bekommen das Fenster nie (Konzept Abschnitt 3).
		// Bewusst ohne Marke: ein Gast hat keinen Merkzettel zu fuehren, und der
		// Riegel steht vor jeder Datei- und Einstellungslesung.
	...
	private function readCatalogue(): array {
		...
		foreach ($decoded as $version => $entries) {
			...
			// x.y.z-Schema kommt das nicht vor, aber die Vorlage wandert durch
			// die ganze Flotte — dort faellt ein stilles continue niemandem auf.
