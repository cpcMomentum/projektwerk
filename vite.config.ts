...
export default defineConfig(({ mode }) => ({
	...
	define: {
		...
		appName: JSON.stringify('PROJEKTWERK'),
	...
	build: {
		...
		lib: {
			...
			name: 'projektwerk',
			formats: ['iife'],
			fileName: () => 'js/projektwerk-main.js',
		},
		rollupOptions: {
			output: {
				assetFileNames: (assetInfo) => {
					if (assetInfo.name && assetInfo.name.endsWith('.css')) {
						return 'css/projektwerk-main.css'
					}
					return 'css/[name][extname]'
				},
				// Kein `globals`-Eintrag: der galt nur fuer externalisierte Abhaengigkeiten,
				// und `vue` ist hier bewusst NICHT external — im NC-IIFE-Bundle muss Vue
				// mitgeliefert werden. Der Eintrag war wirkungslos.
			},
		},
	},
}))
