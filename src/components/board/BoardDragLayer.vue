<template>
	<!--
		**Die einzige Stelle, an der `vue-draggable-plus` importiert wird** (Plan
		...
		für sofortiges Feedback, danach rechnet `onDrop` die Nachbar-IDs und meldet
		sie hoch. `store.moveTicket` lädt neu, und der frische Stand ersetzt die
		Kopie über den `tickets`-Watcher — genau derselbe Kommandoweg wie das Menü.
	-->
	<VueDraggable
		v-model="lokal"
		:group="{ name: 'pw-board' }"
		...
		@add="onDrop"
		@update="onDrop">
		<template v-for="ticket in lokal" :key="ticket.id">
			<slot :ticket="ticket" />
		</template>
	</VueDraggable>
</template>
 ...
 *
 * Umschließt die Karten **einer** Spalte. Alle Layer teilen die Gruppe
 * `pw-board`, deshalb wandern Karten über Spaltengrenzen. Das Ziehen per langem
 * Antippen (`delay` + `delayOnTouchOnly`) lässt vertikales Scrollen auf dem
 * Touchgerät erhalten; mit der Maus zieht die Bewegung sofort, ein Klick ohne
 * Bewegung öffnet weiterhin die Karte.
 */
